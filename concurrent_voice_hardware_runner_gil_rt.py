...
- 음성 인식 → 명령
- 로봇팔 제어
- ✅ 손이 실제로 움직인 프레임부터 다이너믹셀이 실제로 움직이기까지의 반응시간(ms) 측정
- ✅ 반응시간을 rt_log.csv 로 저장
"""
...
class ConcurrentVoiceHardwareRunner:
    ...
    def __init__(self, model_path=None, hardware_config_path='hardware_config.json',
                 arduino_port="/dev/arduino", test_mode=False, target_fps=120.0,
        ...
        self.hand_motion_threshold_px = 1.0
        self._prev_rt_left = None
        self._prev_rt_right = None
        self._rt_cooldown = 0.5
        self._rt_cooldown_until = 0.0
        self.motion_onset_timeout_ms = 1000.0
        self.motion_pos_delta_thresh = 1
        self.motion_check_servo_idx = 0
        self._rt_counter = 0
        self._rt_accum_ms = 0.0
        self._rt_min_ms = float('inf')
        self._rt_max_ms = 0.0
        self.rt_report_every = 1

        # CSV logging
        self.rt_csv_path = "rt_log.csv"
        self._rt_csv_lock = threading.Lock()
        self._init_rt_csv()

    # CSV init
    def _init_rt_csv(self):
        need_header = not os.path.exists(self.rt_csv_path)
        try:
            with open(self.rt_csv_path, "a", newline="") as f:
                writer = csv.writer(f)
                if need_header:
                    writer.writerow([
                        "host_time_sec",
            ...
            self.logger.error(f"RT CSV init failed: {e}")

    def _write_rt_csv(self, rt_ms: float, servo_id: Optional[int],
                      left_xy, right_xy, goal_positions, t_motion_start: float):
        # t_motion_start는 perf_counter 기반이라 절대시간이 아님 → host_time_sec는 time.time()으로
        host_ts = time.time()
        lx = left_xy[0] if (left_xy is not None and np.isfinite(left_xy).all()) else ""
        ly = left_xy[1] if (left_xy is not None and np.isfinite(left_xy).all()) else ""
        rx = right_xy[0] if (right_xy is not None and np.isfinite(right_xy).all()) else ""
        ry = right_xy[1] if (right_xy is not None and np.isfinite(right_xy).all()) else ""
        goal_str = ""
        if goal_positions is not None:
            try:
                goal_str = ";".join(str(int(v)) for v in goal_positions)
            except Exception:
                goal_str = ""
        with self._rt_csv_lock:
            try:
                with open(self.rt_csv_path, "a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        f"{host_ts:.6f}",
                        f"{rt_ms:.3f}",
                        servo_id if servo_id is not None else "",
                        f"{lx:.2f}" if lx != "" else "",
                        f"{ly:.2f}" if ly != "" else "",
                        f"{rx:.2f}" if rx != "" else "",
                        f"{ry:.2f}" if ry != "" else "",
                ...
                self.logger.error(f"RT CSV write failed: {e}")
    ...
    def _measure_motion_onset_since(self, t_capture: float) -> Optional[tuple]:
        """
        모터가 실제로 움직이기 시작한 시점을 잡아
        (반응시간_ms, 사용된_servo_id) 를 리턴.
        """
        if self.test_mode or not DynamixelSDK_available:
            return None
        ...
        base_pos = self._dxl_read_present_position(sid)
        while time.perf_counter() < deadline:
            vel = self._dxl_read_present_velocity(sid)
            if vel is not None and vel != 0:
                t_on = time.perf_counter()
                return ((t_on - t_capture) * 1000.0, sid)
            if base_pos is not None:
                pos = self._dxl_read_present_position(sid)
                if pos is not None and abs(pos - base_pos) >= self.motion_pos_delta_thresh:
                    t_on = time.perf_counter()
                    return ((t_on - t_capture) * 1000.0, sid)
            time.sleep(0.001)
        return None

    def _rt_record(self, ms: float):
        self._rt_counter += 1
        ...
        if ms > self._rt_max_ms:
            self._rt_max_ms = ms
        if self._rt_counter >= self.rt_report_every:
            avg = self._rt_accum_ms / self._rt_counter
            print(f"[RT] avg={avg:.2f} ms | min={self._rt_min_ms:.2f} | max={self._rt_max_ms:.2f} (n={self._rt_counter})")
            self._rt_counter = 0
            self._rt_accum_ms = 0.0
            self._rt_min_ms = float('inf')
            self._rt_max_ms = 0.0

    def _detect_hand_motion_start(self, left_xy, right_xy, left_tcap, right_tcap) -> Optional[float]:
        now = time.perf_counter()
        if now < self._rt_cooldown_until:
            return None

        motion_ts = None
        # left
        if left_xy is not None and np.isfinite(left_xy).all() and left_tcap is not None:
            if self._prev_rt_left is not None:
                px, py, _ = self._prev_rt_left
                if np.isfinite([px, py]).all():
                    ...
                    if dist >= self.hand_motion_threshold_px:
                        motion_ts = left_tcap
            self._prev_rt_left = (left_xy[0], left_xy[1], left_tcap)
        else:
            self._prev_rt_left = None

        # right
        if right_xy is not None and np.isfinite(right_xy).all() and right_tcap is not None:
            if self._prev_rt_right is not None:
                px, py, _ = self._prev_rt_right
                if np.isfinite([px, py]).all():
                    dx = right_xy[0] - px
                    dy = right_xy[1] - py
                    dist = (dx*dx + dy*dy) ** 0.5
                    if dist >= self.hand_motion_threshold_px:
                        if motion_ts is None or right_tcap < motion_ts:
                            motion_ts = right_tcap
            self._prev_rt_right = (right_xy[0], right_xy[1], right_tcap)
        else:
            self._prev_rt_right = None

        if motion_ts is not None:
            self._rt_cooldown_until = now + self._rt_cooldown
            return motion_ts
        return None
        ...
        try:
            while self.running:
                ...
                if self.hand_tracking_enabled or self.show_display:
                    ...
                    left_handedness = right_handedness = None
                ...
                if robot_stopped:
                    ...
                    t_motion_start = self._detect_hand_motion_start(
                        np.array(left_features), np.array(right_features),
                        left_tcap, right_tcap
                    )
                else:
                ...
                if (self.rt_enabled and hand_tracking_enabled and self.model is not None and
                        t_motion_start is not None and sent_ok and
                        not self.test_mode and DynamixelSDK_available and hasattr(self, 'packet_handler')):
                    rt_pair = self._measure_motion_onset_since(t_motion_start)
                    if rt_pair is not None:
                        rt_ms, rt_servo = rt_pair
                        self._rt_record(rt_ms)
                        # CSV 저장
                        self._write_rt_csv(
                            rt_ms=rt_ms,
                            servo_id=rt_servo,
                            left_xy=np.array(left_features),
                            right_xy=np.array(right_features),
                            goal_positions=final_positions,
...
def main():
    parser = argparse.ArgumentParser(description="Concurrent Voice + Hardware Runner (with RT CSV)")
