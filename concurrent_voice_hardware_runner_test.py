...
import ctypes.util
import struct
from typing import Optional, List, Sequence, Callable, Tuple
...
LED_COMMAND_MAP = {
    "LED_ON": "ON", "LED_OFF": "OFF", "LED_BRIGHTER": "UP", "LED_DIMMER": "DOWN",
    "LED_RED": "RED", "LED_GREEN": "GREEN", "LED_BLUE": "BLUE",
...
def fuzzy_match_word(text: str, target: str, max_dist: int) -> bool:
    t, s = normalize(target), normalize(text)
    n = len(t)
    if n == 0: return False
    if len(s) < n: return levenshtein(s, t) <= max_dist
    if t in s: return True
    for i in range(len(s)-n+1):
        if levenshtein(s[i:i+n], t) <= max_dist: return True
    return False
...
def detect_cmd_interim(text: str) -> Optional[str]:
    ...
    priority = ["EXIT","EMERGENCY_RESET","STOP","GO_HOME","TRACKING_ON",
                "LED_OFF","LED_ON","LED_BRIGHTER","LED_DIMMER",
                "LED_RED","LED_GREEN","LED_BLUE","LED_YELLOW","LED_WHITE","LED_RAINBOW"]
    for cmd in priority:
        if quick_contains(text, COMMAND_SYNONYMS[cmd]): return cmd
    return None
...
class MicrophoneStream:
    def __init__(self, mic_index: Optional[int], mic_hint: str, debug: bool):
        ...
        self._ratecv_state = None; self._hw_rate = None; self._hw_channels = 1
        self.closed = True
    ...
    def _to_mono_16k(self, data: bytes) -> bytes:
        ...
        if self._hw_rate != TARGET_RATE:
            pcm, self._ratecv_state = audioop.ratecv(pcm, BYTES_PER_SAMPLE, 1,
...
def build_client_and_config(single_utter: bool):
    ...
    phrases = list(set(
        ...
        ["불 꺼 줘", "불켜 줘", "라이트 오프", "라이트 온", "라이트오프", "라이트온", "종료해", "끝내", "그만"] +
...
class ConcurrentVoiceHardwareRunner:
    # Dynamixel control table (X-series typical)
    ...
    def __init__(self, model_path=None, hardware_config_path='hardware_config.json',
        ...
        if torch.cuda.is_available():
            try: self.logger.info(f"CUDA Device: {torch.cuda.get_device_name()}")
    ...
    def ensure_arduino_connected(self, force: bool = False) -> bool:
        ...
        with self._arduino_lock:
            if self.arduino and getattr(self.arduino, "is_open", False): return True
            now = time.time()
        ...
        with self._arduino_lock:
            if candidate and getattr(candidate, "is_open", False):
                ...
                return True
            self.arduino = None; self._last_arduino_ping = 0.0
            return False
    ...
    def send_arduino_command(self, cmd: str, quiet: bool = False) -> bool:
        with self._arduino_lock:
            if not self.arduino or not getattr(self.arduino, "is_open", False):
                if not quiet: self.logger.info("❌ Arduino not connected")
                return False
            try:
                self.arduino.write((cmd + "\n").encode()); self.arduino.flush()
                if not quiet: self.logger.info(f"👉 Arduino: {cmd}")
                return True
            except Exception as e:
                if not quiet: self.logger.error(f"❌ Arduino send failed: {e}")
                return False
    ...
    def _cache_and_fill(self, camera_name: str, xy: List[float]) -> List[float]:
        ...
        if cache is None:
            cache = {'xy':[np.nan,np.nan],'t':0.0}; self.coord_cache[camera_name]=cache
        arr = np.asarray(xy, dtype=np.float32)
        if np.isfinite(arr).all():
            cache['xy']=[float(arr[0]), float(arr[1])]; cache['t']=now
            return cache['xy']
        age = now - cache['t']
        ...
        try:
            ...
            if 'feedforward' in al and 'res' not in al:
                ...
                if not hidden_sizes and not has_ln:
                    ...
                    self.model = ConfigurableFeedforward(4, 4, hidden_sizes, float(config.get('dropout', 0.0))).to(self.device)
                    self.logger.info(f"Created ConfigurableFeedforward with hidden_sizes={hidden_sizes}")
            elif 'res' in al:
                self.model = ResFeedforward(4, 4, config.get('dropout', 0.0)).to(self.device)
                self.logger.info(f"Created ResFeedforward model: {arch}")
            else:
                self.model = SimpleTransformer(
                    4, 4, config.get('d_model', 8), config.get('nhead', 1),
    ...
    def setup_cameras(self):
        ...
        if right_cfg.get('enabled', True):
            self.cameras['right'] = self._setup_single_camera('right', right_cfg.get('id', 2), right_cfg)
        else:
            self.cameras['right'] = None

    def _setup_single_camera(self, name, cam_id, cfg):
        try:
            ...
            if cam.isOpened():
                ...
                self.logger.info(f"{name.title()} camera ready (id={cam_id})")
                return cam
            else:
                self.logger.warning(f"{name.title()} camera not available (id={cam_id})")
                return None
        except Exception as e:
            self.logger.error(f"Failed to setup {name} camera: {e}")
            return None

    def setup_mediapipe(self):
        self.mp_hands = mp.solutions.hands
        self.hands_left = self.mp_hands.Hands(static_image_mode=False, max_num_hands=1,
                                              min_detection_confidence=0.2, min_tracking_confidence=0.1,
                                              model_complexity=0)
        self.hands_right = self.mp_hands.Hands(static_image_mode=False, max_num_hands=1,
                                               min_detection_confidence=0.2, min_tracking_confidence=0.1,
                                               model_complexity=0)
    ...
    def setup_servos(self):
        ...
        port = follower_cfg.get('port', '/dev/follower_arm')
        baudrate = follower_cfg.get('baudrate', 1000000)
        ...
        p_gain_values = follower_cfg.get('position_p_gains', [])
        if p_gain_values:
                ...
                if i < len(p_gain_values):
                    try:
                        dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(
                            self.port_handler, servo_id, self.ADDR_POSITION_P_GAIN, int(p_gain_values[i])
                        )
                        if dxl_comm_result != COMM_SUCCESS: self.logger.warning(f"Failed to set P Gain for motor {servo_id}: Comm error")
                        elif dxl_error != 0: self.logger.warning(f"Failed to set P Gain for motor {servo_id}: Error {dxl_error}")
                        else: self.logger.info(f"Position P Gain set to {int(p_gain_values[i])} for motor {servo_id}")
                    except Exception as e:
                        self.logger.error(f"Exception while setting P Gain for motor {servo_id}: {e}")
        else:
    ...
    def enable_torque(self):
        ...
        for sid in self.servo_ids:
            try:
                dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, sid, self.ADDR_TORQUE_ENABLE, 1)
    ...
    def disable_torque(self):
        ...
        for sid in self.servo_ids:
            try:
                dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, sid, self.ADDR_TORQUE_ENABLE, 0)
    ...
    def send_servo_commands(self, positions):
        ...
        if self.last_sent_positions is not None:
            ...
            if not positions_changed: return True
        try:
            ...
            if success:
                ...
                if self.consecutive_failures >= self.max_consecutive_failures: self.emergency_stop = True
            return success
        except Exception as e:
        ...
        while self.capture_active.is_set():
            try:
                ...
                if ret:
                    ...
                    with self._frame_queue_lock:
                        try:
                            self.frame_queues[name].put_nowait((frame, t_capture))
                        except queue.Full:
                            try:
                ...
                self.logger.error(f"Camera {name} error: {e}")
                break

    def stop_camera_threads(self):
        self.capture_active.clear()
        for name, th in self.camera_threads.items():
            th.join(timeout=1.0)
    ...
    def get_latest_frames(self) -> dict:
        ...
        for name in self.cameras.keys():
            try:
                ...
                with self._frame_queue_lock:
                    while True:
                        try: item = self.frame_queues[name].get_nowait()
                        except Empty: break
                frames[name] = item  # (frame, t_capture) or None
            except KeyError:
                frames[name] = None
        return frames

    def extract_hand_features(self, frame_with_ts, camera_name) -> Tuple[List[float], Optional[np.ndarray], bool, Optional[float]]:
        """
        Returns: (xy, display_frame, detected_now, t_capture)
        """
        ...
        for name, item in frames.items():
            if item is not None:
                ...
                if self.robot_stopped:
                    status, color = "STOPPED", (0,165,255)
                elif self.hand_tracking_enabled:
                    status, color = "TRACKING", (0,255,0)
                else:
                    status, color = "HOME", (255,255,0)
                cv2.putText(display_frame, status, (20,35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                if self.emergency_stop:
                    cv2.rectangle(display_frame, (5,5), (635,475), (0,0,255), 5)
                    cv2.putText(display_frame, "EMERGENCY!", (200,240), cv2.FONT_HERSHEY_BOLD, 1.5, (0,0,255), 4)
                if self.hand_tracking_enabled:
                    feats = left_features if name == 'left' else right_features
                    x, y = feats
                    if np.isfinite([x,y]).all():
                        col = (0,255,0) if name == 'left' else (0,255,255)
                        cv2.circle(display_frame, (int(x), int(y)), 10, col, -1)
                cv2.imshow(f"{name.title()} Camera", display_frame)
    ...
    def _process_speech_stream(self, description: str, timeout: float, handler: Callable[[object], bool],
        ...
        stream_ctx = None
        try:
            stream_ctx, responses = start_stream(self.mic_index, self.mic_hint, self.debug, for_command=for_command)
        except Exception as e:
        ...
        try:
            for response in responses:
                ...
                if timeout and time.time() - start_time > timeout:
                    if timeout_message: print(timeout_message)
                    return "timeout"
                if response.results and handler(response): return "success"
            return "no_match"
        except Exception as e:
            ...
            if stream_ctx:
                ...
                except Exception as close_err: self.logger.error(f"Error closing {description} stream: {close_err}")
    ...
    def voice_recognition_thread(self):
            ...
            for res in resp.results:
                if res.alternatives and not res.is_final:
                    ...
                    if text and detect_wake_interim(text):
                        self.send_arduino_command("LED_EFFECT:8", quiet=True); return True
            return False
        def command_handler(resp)->bool:
            for res in resp.results:
                if res.alternatives:
                    text = res.alternatives[0].transcript.strip()
                    if text:
        ...
        while self.running and self.voice_active:
            try:
                wake_status = self._process_speech_stream("wake", 10.0, wake_handler, for_command=False)
                if wake_status in ("start_error","error"):
                    consecutive_errors += 1
                    if consecutive_errors >= max_consecutive_errors:
                        print("⚠️ Too many voice recognition errors, stopping voice thread")
                        self.voice_active = False; break
                    time.sleep(2.0); continue
                if wake_status == "stopped": break
                if wake_status != "success": continue
                print("✅ Wake word detected - Listening for command...")
                cmd_status = self._process_speech_stream("command", 3.0, command_handler, for_command=False,
                                                         timeout_message="⏰ Command timeout")
                if cmd_status == "stopped": break
                if cmd_status == "error": consecutive_errors += 1
    ...
    def handle_voice_command(self, command):
        ...
        if command == "EXIT":
            print("🛑 Exit command received"); self.running = False
        elif command == "EMERGENCY_RESET":
            print("🔄 EMERGENCY RESET - Recovering system...")
            with self._robot_state_lock:
                if self.emergency_stop:
                    if not self.test_mode:
                        temp = self.emergency_stop; self.emergency_stop = False
                        if self.move_to_position(self.safe_holding_position):
                            self.consecutive_failures = 0; self.robot_stopped = False; self.hand_tracking_enabled = False
                            print("✅ System RECOVERED - Emergency stop cleared, moved to HOME")
                        else:
                            self.emergency_stop = temp; print("❌ Recovery FAILED - still in emergency stop")
                    else:
                        self.emergency_stop = False; self.consecutive_failures = 0; print("✅ System RECOVERED (test mode)")
                else: print("ℹ️ System is not in emergency stop")
        elif command == "STOP":
            with self._robot_state_lock:
                self.robot_stopped = True; self.hand_tracking_enabled = False
            self.send_arduino_command("LED_EFFECT:11", quiet=True); print("⏹️ Robot STOPPED")
        elif command == "GO_HOME":
            with self._robot_state_lock:
                self.robot_stopped = False; self.hand_tracking_enabled = False; self.consecutive_failures = 0
            self.send_arduino_command("LED_EFFECT:10", quiet=True); print("🏠 Going HOME")
            if not self.test_mode: self.move_to_position(self.safe_holding_position)
        elif command == "TRACKING_ON":
            with self._robot_state_lock:
                self.robot_stopped = False; self.hand_tracking_enabled = True
            self.send_arduino_command("LED_EFFECT:9", quiet=True); print("📷 Hand tracking ENABLED")
        elif command in LED_COMMAND_MAP:
            arduino_cmd = LED_COMMAND_MAP[command]
            if self.ensure_arduino_connected(force=True):
                if not self.send_arduino_command(arduino_cmd):
                    print(f"⚠️ Arduino 전송 실패 - 재시도 ({arduino_cmd})"); self._mark_arduino_disconnected()
            else:
    ...
    def _rt_record_and_maybe_print(self, ms: float):
        self._rt_counter += 1; self._rt_accum_ms += ms
        if ms < self._rt_min_ms: self._rt_min_ms = ms
        if ms > self._rt_max_ms: self._rt_max_ms = ms
        if self._rt_counter >= self.rt_every_n:
            avg = self._rt_accum_ms / self._rt_counter
            print(f"[Reaction Time] avg={avg:.2f} ms | min={self._rt_min_ms:.2f} ms | max={self._rt_max_ms:.2f} ms (n={self._rt_counter})")
            self._rt_counter = 0; self._rt_accum_ms = 0.0
            self._rt_min_ms = float('inf'); self._rt_max_ms = 0.0
            ...
            while self.running:
                ...
                if should_ping and arduino_ref:
                    if self.send_arduino_command("STATUS", quiet=True):
                        with self._arduino_lock: self._last_arduino_ping = time.time()
                    else:
                        self._mark_arduino_disconnected()

                frames = self.get_latest_frames()

                # === Feed frames to MediaPipe (in parallel) ===
                if self.hand_tracking_enabled or self.show_display:
                    lf = self.hand_tracking_executor.submit(self.extract_hand_features, frames.get('left'), 'left')
                    rf = self.hand_tracking_executor.submit(self.extract_hand_features, frames.get('right'), 'right')
                    left_features, left_processed, left_detected, tcap_left = lf.result()
                    right_features, right_processed, right_detected, tcap_right = rf.result()
                    if left_processed is not None: frames['left'] = (left_processed, tcap_left)
                    if right_processed is not None: frames['right'] = (right_processed, tcap_right)
                else:
                    left_features = [np.nan, np.nan]; right_features = [np.nan, np.nan]
                ...
                # choose start t_capture from actually detected cams (min of the two)
                t_capture_candidates = []
                if left_detected and tcap_left is not None: t_capture_candidates.append(tcap_left)
                if right_detected and tcap_right is not None: t_capture_candidates.append(tcap_right)
                t_capture = min(t_capture_candidates) if t_capture_candidates else None
                ...
                if (self.measure_reaction_time and hand_tracking_enabled and self.model is not None and
                    ...
                    # if None, we simply skip this sample (timeout / unsupported)
    ...
    def cleanup(self):
        ...
        if arduino_ref:
            try:
                ...
                if self.send_arduino_command("OFF"):
                    time.sleep(0.5); self.logger.info("Arduino LED turned off")
            except Exception as e:
        ...
        if not self.test_mode and DynamixelSDK_available and hasattr(self, 'port_handler'):
            try:
                self.port_handler.closePort(); self.logger.info("Servo port closed")
            except Exception as e:
...
def main():
    ...
    print("  Arduino LEDs: '켜' / '꺼', 밝기 '밝게' / '어둡게', 색상 '빨간불' 등")
    print("  System: '종료'")
    print()
