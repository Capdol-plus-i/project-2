...
# -*- coding: utf-8 -*-
"""
Concurrent Voice + Hardware Runner  (REACTION-TIME OBSERVATION ONLY PATCH)
- 관측 전용 반응속도 측정 추가:
  * 손 검출된 프레임에서만 측정 무장
  * 서보 명령 송신 직후 t0 기록
  * Present Position(addr=132) 변화 감지로 움직임 시작 t1 포착
  * 제어 파라미터/구동 로직은 그대로 (측정만 수행)
"""
...
class ConcurrentVoiceHardwareRunner:
    def __init__(self, model_path=None, hardware_config_path='hardware_config.json',
        ...
        # ========== Reaction-Time Observation (NEW) ==========
        # 측정은 관측 전용. 하드웨어 피드백이 없으면 자동 비활성화
        self.rt_enabled = (not self.test_mode) and DynamixelSDK_available
        self.rt_threshold_ticks = 6          # 움직임 시작으로 간주할 최소 위치 변화(ticks)
        self.rt_timeout_s = 2.0              # 무장 후 타임아웃
        self.rt_samples_max_report = 20      # N개 수집 시 요약 출력
        self.rt_pending = False              # 현재 측정 무장 여부
        self.rt_t0 = None                    # 명령 송신 직후 시각
        self.rt_baseline = None              # 무장 시점의 현재 위치(피드백)
        self.rt_samples = []                 # 수집된 반응시간(ms)
        self._last_feedback_positions = None # 직전 루프에 읽은 현재 위치
        # =====================================================
    ...
    def send_arduino_command(self, cmd: str, quiet: bool = False) -> bool:
        with self._arduino_lock:
            if not self.arduino or not getattr(self.arduino, "is_open", False):
                if not quiet: self.logger.info(f"❌ Arduino not connected")
                return False
            try:
    ...
    def _cache_and_fill(self, camera_name: str, xy: List[float]) -> List[float]:
        now = time.time()
        cache = self.coord_cache.get(camera_name) or {'xy': [np.nan, np.nan], 't': 0.0}
        ...
        try:
            ...
            if 'feedforward' in arch_lower and 'res' not in arch_lower:
                ...
                if not hidden_sizes and not has_layer_norm:
                    ...
                    self.logger.info("Created ConfigurableFeedforward model with hidden_sizes=%s, dropout=%.3f",
                                     hidden_sizes, dropout)
            elif 'res' in arch_lower:
                self.model = ResFeedforward(input_dim=4, output_dim=4, dropout=config.get('dropout', 0.0)).to(self.device)
    ...
    def setup_mediapipe(self):
        ...
        self.hands_left = self.mp_hands.Hands(
            ...
            min_detection_confidence=0.3, min_tracking_confidence=0.1,
            model_complexity=0
        )
    ...
    def setup_servos(self):
        ...
        # === Reaction-time: prepare GroupSyncRead for present position (addr=132, 4 bytes) ===
        try:
            self.present_pos_addr = robot_config.get('addr_present_position', 132)
            self.group_sync_read = GroupSyncRead(self.port_handler, self.packet_handler, self.present_pos_addr, 4)
            for sid in self.servo_ids:
                self.group_sync_read.addParam(sid)
            self.rt_enabled = True  # we have feedback → can measure
            self.logger.info("Reaction-time reader initialized (present position)")
        except Exception as e:
            self.rt_enabled = False
            self.logger.warning(f"Reaction-time reader init failed: {e} (measurement disabled)")
    ...
    def read_servo_positions(self) -> Optional[List[int]]:
        """Read present positions for all servos (for reaction-time only)."""
        if self.test_mode or not DynamixelSDK_available or not hasattr(self, 'group_sync_read'):
            return None
        try:
            ...
            positions = []
            for sid in self.servo_ids:
                if not self.group_sync_read.isAvailable(sid, self.present_pos_addr, 4):
                    return None
                pos = self.group_sync_read.getData(sid, self.present_pos_addr, 4)
    ...
    def clamp_positions(self, positions):
        ...
        safe_positions = []
        for i, pos in enumerate(positions):
            min_pos = self.safe_zone_min[i]; max_pos = self.safe_zone_max[i]
        ...
        for camera_name, camera in self.cameras.items():
            if camera is not None:
                ...
                thread = threading.Thread(
                    target=self._camera_capture_thread, args=(camera_name, camera), daemon=True
                )
                thread.start()
        ...
        try:
            ...
            if results.multi_hand_landmarks:
                ...
                if 0 <= index_tip.x <= 1 and 0 <= index_tip.y <= 1:
                    h, w = frame.shape[:2]
                    x = index_tip.x * w; y = index_tip.y * h
                    if 0 <= x <= w and 0 <= y <= h:
    ...
    def update_display(self, frames, left_features, right_features):
        ...
        for camera_name, frame in frames.items():
            if frame is not None:
                display_frame = frame
                cv2.rectangle(display_frame, (10, 10), (320, 130), (0, 0, 0), -1)
                if self.robot_stopped:
                    robot_status = "STOPPED"; robot_color = (0, 165, 255)
                elif self.hand_tracking_enabled:
                    robot_status = "TRACKING"; robot_color = (0, 255, 0)
                else:
                    robot_status = "HOME"; robot_color = (255, 255, 0)
    ...
    def voice_recognition_thread(self):
            ...
            for res in response.results:
                if res.alternatives:
                    ...
                    if text:
                        ...
                        if cmd:
                            self.handle_voice_command(cmd); return True
            return False
        while self.running and self.voice_active:
            try:
                ...
                if wake_status in ("error",): consecutive_errors += 1; continue
                if wake_status == "stopped": break
                if wake_status != "success": continue
                print("✅ Wake word detected - Listening for command...")
                cmd_status = self._process_speech_stream(
                    "command", 3.0, command_handler, for_command=False, timeout_message="⏰ Command timeout",
                )
        ...
        if command == "EXIT":
            ...
            if self.emergency_stop:
                if not self.test_mode:
                    ...
                    self.emergency_stop = False; self.consecutive_failures = 0
            ...
            self.robot_stopped = False; self.hand_tracking_enabled = False
            self.consecutive_failures = 0
    ...
    def _hand_detected(self, left_features, right_features) -> bool:
        lx, ly = left_features; rx, ry = right_features
        return (np.isfinite([lx, ly]).all()) or (np.isfinite([rx, ry]).all())

    def _reaction_time_arm(self):
        """무장: 현재 피드백을 기준으로 t0 기록. (손 검출 + 추적 ON 조건에서만)"""
        if not self.rt_enabled or self.rt_pending:
            return
        baseline = self._last_feedback_positions
        if baseline is None:
            return
        self.rt_t0 = time.perf_counter()
        self.rt_baseline = baseline[:]
        self.rt_pending = True

    def _reaction_time_check_and_maybe_commit(self):
        """무장 상태에서 피드백 변화가 임계치를 넘으면 t1 기록 → 샘플 확정."""
        if not self.rt_enabled or not self.rt_pending:
            return
        current = self._last_feedback_positions
        if current is None:
            return
        moved = False
        for a, b in zip(current, self.rt_baseline):
            if abs(a - b) >= self.rt_threshold_ticks:
                moved = True; break
        now = time.perf_counter()
        if moved:
            dt_ms = (now - self.rt_t0) * 1000.0
            self.rt_samples.append(dt_ms)
            self.rt_pending = False
            # N개마다 요약 출력 (관측 전용)
            if len(self.rt_samples) % self.rt_samples_max_report == 0:
                arr = np.array(self.rt_samples[-self.rt_samples_max_report:], dtype=np.float64)
                print(f"[Reaction Time] avg={arr.mean():.2f} ms | min={arr.min():.2f} ms | max={arr.max():.2f} ms (n={len(arr)})")
        else:
            # 타임아웃: 측정 취소 (관측만, 제어 영향 없음)
            if (now - self.rt_t0) > self.rt_timeout_s:
                self.rt_pending = False
    ...
    def run_concurrent_loop(self):
        ...
        try:
            while self.running:
                ...
                # --- Read servo feedback early for reaction-time baseline/commit ---
                self._last_feedback_positions = self.read_servo_positions()

                # --- Frames & hand extraction ---
                frames = self.get_latest_frames()
                if self.hand_tracking_enabled or self.show_display:
                    left_future = self.hand_tracking_executor.submit(self.extract_hand_features, frames.get('left'), 'left')
                    right_future = self.hand_tracking_executor.submit(self.extract_hand_features, frames.get('right'), 'right')
                ...
                # --- Reaction-time: arm if hand detected & tracking ON ---
                if self.hand_tracking_enabled and self._hand_detected(left_features, right_features):
                    # 명령 송신 직후 시간을 t0로 잡기 위해, 송신 전에 무장해두고
                    # 송신 직후 t0를 실제로 찍는다. (아래에서 직후에 찍음)
                    can_arm = (self.rt_enabled and not self.rt_pending and self._last_feedback_positions is not None)
                else:
                    can_arm = False

                # --- Send command (unchanged) ---
                # 송신 '직후' t0 기록 → 카메라→추론→명령 송신까지 포함한 시작점
                if can_arm:
                    # 잠정 무장(베이스라인은 이미 루프 초반 피드백으로 확보)
                    self.rt_t0 = None  # 직후에 찍기 위해 비워둠
                    self.rt_baseline = self._last_feedback_positions[:]
                    self.rt_pending = True

                sent_ok = self.move_to_position(final_positions)

                if can_arm and self.rt_pending:
                    # 명령 송신 직후 t0 확정
                    self.rt_t0 = time.perf_counter()

                # --- Reaction-time commit check ---
                self._last_feedback_positions = self.read_servo_positions()
                self._reaction_time_check_and_maybe_commit()
