    ...
    if os.path.exists(config_file):
        try:
            with open(config_file, 'r') as f:
                config = json.load(f)
                leader_config = config.get('robot_arms', {}).get('leader', {})
                return {
                    'port': leader_config.get('port', default_config['port']),
                    'baudrate': leader_config.get('baudrate', default_config['baudrate'])
                }
...
def show_help():
    ...
    print_colored("\n🎮 Leader Arm Controller - Available Commands:", Colors.HEADER)
...
def cleanup_and_exit():
    ...
    print_colored("👋 Leader Arm Controller stopped", Colors.GREEN)
